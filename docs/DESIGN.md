...
| `/map` | **マインドマップ編集画面**（`pages/EditorPage.tsx`） | 左=ノードツリーサイドバー（ノード数）／中央=放射状キャンバス（描画・ズーム・生成中ロック・完了時 fitView）＋上部中央=起点ラベル（トップバー）／**左上=生成コントロール（`EditorControls`：展開モード・連想件数・作成/停止/再生成・状態）**／**右上=アカウント（ログイン・保存）**／**右=ノード編集サイドバー（`NodeEditSidebar`：ノード選択時のみ・名前編集/子追加/削除/手動展開）** | T18, T20, T21, T22 |
