...
test('キーワードから自走展開してマインドマップが描画される', async ({ page }) => {
  ...
  // 起点と連想ノードがキャンバスに描画される（ラベルはサイドバーのツリーにも出るため .react-flow に限定）。
  const canvas = page.locator('.react-flow')
  await expect(canvas.getByText('宇宙')).toBeVisible()
  await expect(canvas.getByText('銀河')).toBeVisible()
  await expect(canvas.getByText('惑星')).toBeVisible()
